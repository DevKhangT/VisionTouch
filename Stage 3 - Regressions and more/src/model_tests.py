...
all_model_results = {}
def evaluate_model(preds_test, y_test, ablation_name, model_name, model_parameters):
...
for ablation_name, feature_columns in ablations.items():
    ...
    evaluate_model(linear_preds, y_test, ablation_name, "linear", None)
...
for (ablation_name, model_name), dfs in all_model_results.items():
    ...
    combined_df.to_csv(f"{dataset}_prediction_results_{ablation_name}_{model_name}.csv", index=False)
    print(f"Saved prediction results for {ablation_name} - {model_name} to {dataset}_prediction_results_{ablation_name}_{model_name}.csv")
# --------------------------------------------------
...
model_results_df.to_csv(f"{dataset}_stage3_models_results.csv", index=False)
...
print(f"\nSaved combined model results to: {dataset}_stage3_models_results.csv")
